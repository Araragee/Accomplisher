import type { WfhLog, Target } from '../types';

export interface CoverageRow extends Target {
  logged: number;
  required: number;
  progress: number;
  met: boolean;
...
export interface CoverageResult {
  ...
  targetCount: number;
  overallPct: number;
  deficits: CoverageRow[];
}

// Roll WFH hours up against IPCR targets.
export function computeCoverage(wfhItems: WfhLog[], targets: Target[]): CoverageResult {
  ...
  const rows: CoverageRow[] = targets.map((t) => {
