    ...
    <Page width="narrow">
      ...
      <div className="mt-6 space-y-4">
        ...
        <Section title="Your profile" description="Used as the default member and on your reports.">
          <form className="flex flex-col gap-3 sm:flex-row sm:items-end" onSubmit={(e) => { e.preventDefault(); saveProfile(); }}>
        ...
        <Section title="IPCR targets" description="Commitment hours the WFH log and Task Thinker track against.">
          <ul className="divide-y divide-line rounded-lg border border-line">
            {targets.map((t) => (
              <li key={t.id} className="flex items-center gap-3 px-3 py-2.5">
                ...
                <span className="font-mono text-xs tabular-nums text-subtle">{t.requiredHours}h</span>
                ...
                <button type="button" aria-label="Delete target" onClick={() => removeTarget(t.id)} className="rounded-md p-1 text-faint hover:bg-danger-soft hover:text-danger focus-ring">
          ...
          <form onSubmit={saveTarget} className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-[1fr_2fr_auto_auto]">
        ...
        <Section title="Data & storage">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Button variant="secondary" onClick={exportData}>Export backup (.json)</Button>
            <Button variant="danger" onClick={clearData}>Clear logged data</Button>
          </div>
        </Section>
      </div>
    </Page>
