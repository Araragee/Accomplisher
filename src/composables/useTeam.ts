...
import type { Member, WfhLog, Accomplishment } from '../types';
...
export interface UseTeamResult {
  ...
  setEditing: React.Dispatch<React.SetStateAction<EditingMember | null>>;
  saveMember: () => Promise<void>;
  removeMember: (m: Member) => Promise<void>;
  viewMember: (m: Member) => void;
}
...
export function useTeam(): UseTeamResult {
  ...
  const [wfhAll, setWfhAll] = useState<WfhLog[]>([]);
  const [accAll, setAccAll] = useState<Accomplishment[]>([]);
